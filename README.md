...
#### Modeling Pipeline
The dataset was split into 80/20 training and test sets using stratified sampling to preserve the outcome class distribution. Features were standardized using StandardScaler fit exclusively on the training set and applied to both sets, preventing data leakage. This was relevant as age ranged upwards of 80 whereas the rest of our features were binary. To address class imbalance in the training set, RandomUnderSampler was applied after scaling, reducing the majority class to match the minority class size. The test set was left at its natural distribution to reflect real-world conditions. This helped with balancing the training set along with computational speed when training these models.
Three classifiers were trained and evaluated: Logistic Regression, Random Forest, and XGBoost. Hyperparameter tuning was performed for each model using RandomizedSearchCV with 5-fold cross-validation, optimizing for F1 score. 5-fold cross-validation aided in preventing overfitting of the model, and the randomized search allowed us to tune hyperparameters in a less exhaustive, more computationally efficient method. Logistic Regression was tuned over solver, penalty type, and regularization strength across 20 random iterations. Random Forest was tuned over number of estimators, tree depth, split criteria, and feature sampling strategy across 50 iterations. XGBoost was tuned over estimator count, tree depth, learning rate, subsampling ratios, minimum child weight, and gamma across 50 iterations.
Each model was evaluated on the held-out test set using accuracy, precision, recall, F1, ROC-AUC, sensitivity, specificity, NPV, and FNR. ROC and Precision-Recall curves were generated for each model.
#### Initial Models
